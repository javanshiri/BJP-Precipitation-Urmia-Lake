...
In <- function (m, l) {if (m-l+1 > 0 ) {i=m-l+1} else {i=m-l+13}
  return(i)}
#############
RPSS <- read.csv("E:\\Papers-Journals\\papers\\BJP\\output\\monthly\\RPS_RAW.csv")
for ( m in 1:12) {
  jpeg (paste0 (path.plot, m, "RPSS_raw.jpg"))
  layout(matrix(c(1, 2, 3, 4), ncol=2, byrow=TRUE), heights=c(7, 7))
    ...
    barplot(mat, beside=T, ylim=c(-1.8, 0), legend=FALSE, 
